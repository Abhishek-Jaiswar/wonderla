...
const CircularSidebar = () => {
    const items = [
        { id: 1, name: "Land", content: "73 Rides", icon: "/land.svg" },
        { id: 2, name: "Water", content: "54 Rides", icon: "/water.svg" },
        { id: 3, name: "Kids", content: "35 Rides", icon: "/kids.svg" },
    ]
    ...
    return (
        <div className='bg-[#22304A] h-screen'>
            <div className='relative flex items-center justify-center w-[37.5rem] h-[37.5rem] -left-[20rem]'>
                {/* Outer circle */}
                <motion.div
                    className='absolute w-full h-full rounded-full bg-white z-0 overflow-hidden'
                >
                    <motion.div
                        ...
                        animate={{
                            x: radiusInner * Math.cos((getAngle(selected, items.length)) * Math.PI / 149),
                            y: radiusInner * Math.sin((getAngle(selected, items.length)) * Math.PI / 149),
                        }}
                        transition={{
                            ease: 'easeIn',
                            duration: 0.5
                ...
                <motion.div
                    ...
                    animate={{
                        x: radiusPointer * Math.cos((getAngle(selected, items.length)) * Math.PI / 150),
                        y: radiusPointer * Math.sin((getAngle(selected, items.length)) * Math.PI / 150),
                    }}
                    transition={{
                        ease: 'easeInOut',
                        duration: 0.5
                ...
                {items.map((item, index) => {
                    ...
                    return (
                        ...
                        >
                            <div
                                className={`transition-transform duration-500 p-2 cursor-pointer rounded-full ${selected === index ? "scale-125" : "scale-100"
                                    }`}
                            >
                                <Image
                                    src={item.icon}
